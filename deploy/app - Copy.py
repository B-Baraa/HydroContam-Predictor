...
API_KEY = st.secrets.get("OPENWEATHER_API_KEY") or st.sidebar.text_input(
...
with col2:
    ...
    wqi = st.slider("Water Quality Index", 0.0, 100.0, 40.0)
...
if nitrate > 50:
    score += 3
    danger_reasons.append("Excessive nitrate concentration")
elif nitrate > 25:
    score += 1
...
if conductivity > 1000:
    score += 2
    danger_reasons.append("High dissolved salts concentration")
elif conductivity > 500:
    score += 1

if turbidity > 5:
    score += 2
    danger_reasons.append("High turbidity detected")

if coliform > 0:
    score += 3
    danger_reasons.append("Bacterial contamination detected")
...
if st.button("Analyze Water Safety"):
    ...
    if prediction == 1:
        st.error(f"⚠️ HIGH CONTAMINATION RISK\n\nProbability: {probability:.2f}")
    else:
        st.success(f"✅ WATER CONDITIONS APPEAR SAFE\n\nProbability: {probability:.2f}")
    ...
    # FIX: extract a single numeric value per parameter
    safe_values = [
    ...
    # =====================================================
    # 12. THRESHOLD COMPARISON BAR CHART
    # =====================================================
